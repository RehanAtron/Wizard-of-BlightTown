    ...
    void Elongate()
    {
        if (!hasReached)
        {
            for(int i = 0; i < 6; i++)
            {
                ...
                {
                    target.transform.position = Vector2.MoveTowards(target.transform.position,new Vector2(targetPosition.transform.position.x,target.transform.position.y), 8 * Time.deltaTime);
